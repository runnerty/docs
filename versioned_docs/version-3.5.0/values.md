...
By default the environment variables defined in the .env file in the root of the project will be imported.
It is also possible to force the loading of environment variables from another file by specifying their path in the '--env-file' argument in the CLI.

To access the values of these variables we can make use of the `@ENV` [function](functions.md) (alias `@GENV`).
##### Sample, if we define a variable `MYENVVAL=TESTVALUE` we can get its value with
